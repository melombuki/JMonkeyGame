 ...
 * @author Brent Owens
 */
public class TestAI extends SimpleApplication {

    private TestAI.Vehicle target;
    private TestAI.Vehicle vehicle1;
    private Node obstacleNode;
    private Node friendNode;

    public static void main(String[] args) {
        TestAI app = new TestAI();
        app.start();
    }
    ...
    public void simpleInitApp() {
        ...
        // create target
        target = new TestAI.Vehicle(ColorRGBA.Red);
        ...
        target.addControl(new TestAI.MoveStraightControl());
        rootNode.attachChild(target);
        
        // create the vehicle
        vehicle1 = new TestAI.Vehicle(ColorRGBA.Blue);
        ...
        vehicle1.addControl(new TestAI.PersuitAvoidSeparationControl());
        ...
        for (int i=0; i<amount; i++) {
            TestAI.Vehicle obstacle  = new TestAI.Vehicle(ColorRGBA.Yellow);
        ...
        // create neighbours
        for (int i=0; i<amount; i++) {
            TestAI.Vehicle neighbour  = new TestAI.Vehicle(ColorRGBA.Blue);
            neighbour.setLocalTranslation(((float)Math.random())*5f, 0, ((float)Math.random())*5f);
            neighbour.addControl(new TestAI.PersuitAvoidSeparationControl());
    ...
    private void fillObstacals(List<Obstacle> obstacles) {
        for (Spatial s : obstacleNode.getChildren()) {
            if (s instanceof TestAI.Vehicle) {
                TestAI.Vehicle v = (TestAI.Vehicle)s;
     ...
     */
    private void fillNeighbours(TestAI.Vehicle source, List<Obstacle> neighbours, float radius) {
        float r2 = radius*radius;
        for (Spatial s : friendNode.getChildren()) {
            if (s instanceof TestAI.Vehicle && !s.equals(source)) {
                TestAI.Vehicle v = (TestAI.Vehicle)s;
    ...
    private class SeekControl extends TestAI.SimpleControl {
    ...
    private class FleeControl extends TestAI.SimpleControl {
    ...
    private class PersuitControl extends TestAI.SimpleControl {
    ...
    private class EvadeControl extends TestAI.SimpleControl {
    ...
    private class PersuitAndAvoidControl extends TestAI.SimpleControl {
    ...
    private class PersuitSeparationControl extends TestAI.SimpleControl {
    ...
    private class PersuitAvoidSeparationControl extends TestAI.SimpleControl {
    ...
    private class MoveStraightControl extends TestAI.SimpleControl {
